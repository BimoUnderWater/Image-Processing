...
for j in range(3):
 ...
 for i in range(count):
  if i%3 <= 1:
   ...
   if br >= 50 and brdabs <=2:
    #フォルダ内の残りの画像数をカウント
    count-i
    #差分アルゴリズムを定義
    #フレーム間を差分
    im_diff = img_src1.astype(int) - img_src2.astype(int)
    #差分を絶対値に
    im_diff_abs = np.abs(im_diff)
    #差分を正規化
    im_diff_abs_norm = im_diff_abs / im_diff_abs.max() * 255
    #差分処理済み画像を保存する場合
    #cv2.imwrite(os.path.join(dirname, 'no.jpg'),im_diff_abs_norm)
    #二値化処理
    #閾値あり
    ret,thresh1 = cv2.threshold(im_diff_abs_norm,16,255,cv2.THRESH_BINARY)
    ...
    #imgab = cv2.imread('C:/buffer/di.jpg')
    # Lab色空間に変換
    img_Lab = cv2.cvtColor(arr2, cv2.COLOR_BGR2Lab)
    # Lab色空間を分割
    ...
    #cv2.imwrite(os.path.join(dirname, 'output.jpg'), img_greenarea_clean)
    #再ディレイト用カーネル
    ...
    #im = cv2.imread('C:/buffer/di2.jpg')
    #imgray = cv2.cvtColor(im,cv2.COLOR_BGR2GRAY)
    #ret,thresh2 = cv2.threshold(imgray,20,255,0)
    #オブジェクトのカウント
 ...
 os.chdir('C:/result/')
 #行列をCSVとして出力
 with open('resultbr50th131'+str(num)+'.csv', 'w') as f:
     writer = csv.writer(f)
     writer.writerows(a)
